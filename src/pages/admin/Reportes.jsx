...
export default function Reportes() {
  ...
  // 1. Efecto para manejar el cambio de Carrera
useEffect(() => {
  if (carreraSeleccionada) {
    // RESET INMEDIATO de estados dependientes antes de la llamada asíncrona
    setGestiones([]);
    setGestionId('');
    setSemestre('');
    setMaterias([]);
    ...
    // Si deselecciona o vuelve a "-- Seleccionar --"
    setGestiones([]);
    setGestionId('');
    setSemestre('');
    setMaterias([]);
...
// 2. Cargar carreras sin auto-seleccionar el primer elemento
const cargarCarreras = async () => {
  try {
    ...
    setCarreras(data);
    setError('');
    // REMOVIDO: setCarreraSeleccionada(data[0].id); -> Inicia en "" para forzar "Seleccionar"
  } catch (err) {
...
// 3. Cargar gestiones sin auto-seleccionar el primer elemento
const cargarGestiones = async (id) => {
  try {
    const data = await reporteService.obtenerGestiones(id);
    setGestiones(data);
    // REMOVIDO: setGestionId(data[0].id); -> Mantiene el valor en "" ("-- Seleccionar --")
  } catch (err) {
...
// 5. Debounce para la consulta de materias
useEffect(() => {
